...
const Header = ({ activeSection, setActiveSection }: HeaderProps) => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#F5E6D0] border-t-4 md:border-t-8 border-b-4 border-black shadow-2xl">
      <div className="container mx-auto px-4">
        <div className="border-b-2 border-black py-2 hidden md:block">
        ...
        {mobileMenuOpen && (
          <nav className="lg:hidden border-b-4 border-black bg-white/95 backdrop-blur">
            <div className="py-2">
              {menuItems.map((item) => (
                <Button
                  key={item.id}
                  ...
                  className={`w-full font-headline uppercase tracking-wider text-base py-4 rounded-none border-b-2 border-black/10 ${
                    activeSection === item.id
                      ? 'bg-black text-white'
                      : 'text-black hover:bg-black/10'
                  }`}
